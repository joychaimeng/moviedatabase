...
<title>Add Director Movie Relation</title>
   ...
   <body>
      ...
      <h2 style="margin-bottom: 20px;">Add a Director-Movie Relation to Database</h2>
        ...
        <div class="col-md-8">
          ...
          <form action="./add_director_movie.php" method="GET">
            ...
            <?php
              ...
              //dropdown menus
              $director_query = "SELECT id, first, last, dob FROM Director ORDER BY first";
              $director_arr = mysql_query($director_query, $db_connection) or die(mysql_error());
              $selectDirector = "";
              if (mysql_num_rows($director_arr) >=1 ){
                while ($row = mysql_fetch_row($director_arr)) {
                  $id = $row[0];
                  $first = $row[1];
                  $last = $row[2];
                  $dob = $row[3];
                  $selectDirector.="<option value=\"$id\">".$first." ".$last." ( Born on ".$dob." )</option>";
                }
    ...
    <?php
      ...
      if(isset($_GET['submit'])){
          ...
          echo "<div class=\"container\" style=\"margin-top:30px\"><p>New movie-director relation added!</p></div>";
      }
      mysql_close($db_connection);
